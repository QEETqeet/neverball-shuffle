...
#include "st_done.h"
#include "st_shared.h"
#include "st_title.h"   /* roguelike exits to title */
        ...
        if ((jd = gui_hstack(id)))
        {
            ...
            if ((kd = gui_vstack(jd)))
            {
                if (curr_mode() == MODE_ROGUE)
                {
                     ...
                     * Layout:
                     *   big:   "Run 05/10"
                     *   small: "Tour de force - 01"   (set name + level number in set)
                     */
                    char run_str[64];
                    char sub_str[MAXSTR];

                    sprintf(run_str, "Run %02d/%02d",
                            rogue_played(), rogue_run_cap());

                    snprintf(sub_str, sizeof(sub_str), "%s - %s",
                             set_name(curr_set()),
                             level_name(curr_level()));

                    gui_label(kd, run_str,  GUI_LRG, gui_grn, gui_grn);
                    gui_label(kd, sub_str,  GUI_SML, gui_wht, gui_wht);

                    /* message shown below via gui_multi, keep it in message var */
                }
                else
                {
...
int goto_exit(void)
{
    ...
    if (curr_mode() == MODE_ROGUE)
    {
        /* Roguelike: always return to the title screen. */
        dst = &st_title;
        goto_state(dst);
        return 1;
    }
