...
/*
 * In this mod, GOAL_NEXT advances to a random level from the pool
 * instead of the next level in the current set.
 */
static int goal_action(int tok, int val)
{
    ...
    case GOAL_NEXT:
        /* rl:pick a random next level from the pool. */
        if (curr_mode() == MODE_ROGUE)
        {
            if (rogue_has_next())
            {
                ...
                if (l)
                {
                    /* Keep running the same progress session, just new level. */
                    if (progress_play(l))
                        return goto_state(&st_level);
                }
            }
            /* no more levels go to title. */
            progress_stop();
            return goto_state(&st_rogue_done);
        }
        /* Normal mode. */
...
static int goal_gui(void)
{
    ...
    /* shOw how many levels remain. */
    int rogue_mode = (curr_mode() == MODE_ROGUE);
    ...
    if ((root_id = gui_root()))
    {
        if ((id = gui_vstack(root_id)))
        {
            ...
            if (curr_mode() == MODE_CHALLENGE || rogue_mode)
            {
                int coins, score, balls;
                int i;

                /* initial score and balls. */
                ...
                if ((jd = gui_hstack(id)))
                {
                    ...
                    if ((kd = gui_vstack(jd)))
                    {
                        if ((ld = gui_hstack(kd)))
                        {
                            if (rogue_get_balls_enabled() || !rogue_mode)
                            {
                                if ((md = gui_harray(ld)))
                                {
                        ...
                        {
                            char rem_str[64];
                            int  rem = rogue_remaining();

                            if (rem > 0)
                                sprintf(rem_str, _("%d levels left"), rem);
                            else
                                sprintf(rem_str, _("Last level!"));

                            gui_label(kd, rem_str, GUI_SML, gui_yel, gui_yel);
                        }
                        else
                        {
                            if ((ld = gui_harray(kd)))
            ...
            if ((jd = gui_harray(id)))
            {
                if (rogue_mode)
                {
                    /* "Next Level" if pool has more, else "Finish". */
                    if (rogue_has_next())
    ...
    if (!resume)
    {
        ...
        else if (t > 0.05f && coins_id)
        {
            ...
            if (coins > 0)
            {
                ...
                int balls = balls_id ? gui_value(balls_id) : 0;
                ...
                if (balls_id && progress_reward_ball(score + 1))
                {
            ...
            else if (curr_mode() == MODE_ROGUE)
            {
                /*
                 * Coin animation finished in rogue mode.
                 * Auto-advance to the next random level after a short pause.
                 */
                static float rogue_delay = 0.0f;
                rogue_delay += dt;
                if (rogue_delay > 1.5f)
                {
                    rogue_delay = 0.0f;
                    t = 0.0f;
                    goal_action(GOAL_NEXT, 0);
                    return;
                }
            }
            t = 0.0f;
