...
static int help_menu(int id)
{
    ...
    if ((jd = gui_harray(id)))
    {
        help_button(jd, _("Credits"),       HELP_PAGE, PAGE_CONTROLS);
        help_button(jd, _("About the mod"), HELP_PAGE, PAGE_RULES);
...
static const char *more_format(const char *src)
{
    static char buf[1024];
    int i = 0;

    while (*src && i < (int) sizeof (buf) - 1)
    {
        if (*src == '\\')
            buf[i++] = '\n';
        else
...
static int page_more_text(int id, const char *text)
{
    int jd, kd;
    ...
    if ((jd = gui_hstack(id)))
    {
        ...
        {
            gui_space(kd);
            gui_multi(kd, more_format(text), GUI_SML, gui_wht, gui_wht);
            gui_space(kd);
...
static int page_about_mod(int id)
{
    return page_more_text(id, _("This mod turns Neverball into an endless type of game.\n"
                                "\n"
                                "Instead of following the levelset formula, you're able\n"
                                "to pick levelsets and play random levels from them in\n"
                                "challenge mode!\n"
                                "\n"
                                "Its up to you to decide how hard will you go on yourself.\n"));
}

static int page_credits_mod(int id)
{
    return page_more_text(id, _("Credits to the original Neverball developers.\n"
                    ...
                    "Square One - Xaser\n"
                    "Elwood - Dead lock\n"
                    "Skaven / Future Crew - Mercury Rain"));
}
...
static int help_gui(void)
{
    ...
    if ((id = gui_vstack(0)))
    {
        ...
        switch (page)
        {
        case PAGE_RULES:    page_about_mod(id);   break;
        case PAGE_CONTROLS: page_credits_mod(id); break;
