...
/* Roguelike run-progress label shown at the top of the screen. */
static int rogue_prog_id = 0;
...
void hud_init(void)
{
    ...
    /*
     * run-progress label: "Run progress: 12/100"
     * Shown top-center, only visible in MODE_ROGUE.
     * The label text is updated every frame in hud_update().
     */
    if ((rogue_prog_id = gui_label(0, "Run progress: 000/000",
                                   GUI_SML, gui_grn, gui_grn)))
    {
        gui_set_rect(rogue_prog_id, GUI_BOT);
        gui_layout(rogue_prog_id, 0, 1);
    }
}
...
void hud_paint(void)
{
    int rogue = (curr_mode() == MODE_ROGUE);

    if (rogue || curr_mode() == MODE_CHALLENGE)
        gui_paint(Lhud_id);
...
void hud_update(int pulse)
{
    ...
    {
    case MODE_ROGUE:
        /* Show balls as 0 when they are "infinite" (stored as -1). */
        {
            int display_balls = (balls < 0) ? 0 : balls;
            const int balls_enabled = rogue_get_balls_enabled();

            gui_set_color(ball_id,
                          balls_enabled ? gui_wht : gui_blk,
                          balls_enabled ? gui_wht : gui_blk);

            if (gui_value(ball_id) != display_balls)
                gui_set_count(ball_id, display_balls);
        }
    ...
    case MODE_CHALLENGE:
        gui_set_color(ball_id, gui_wht, gui_wht);
    ...
    /* roguelike run-progress label */

    if (curr_mode() == MODE_ROGUE && rogue_prog_id)
    {
        char buf[64];
        int played = rogue_played();
        int cap    = rogue_run_cap();

        sprintf(buf, "Run progress: %d/%d", played, cap);
        gui_set_label(rogue_prog_id, buf);
    }
