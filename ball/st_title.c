...
#include "st_rogue.h"    /* roguelike pre-run setup screen */
...
static int title_action(int tok, int val)
{
    ...
    case TITLE_PLAY:
        /*
         * "Play" goes directly to the roguelike setup screen.
         * If there is no player name yet, ask for it first.
         */
        if (strlen(config_get_s(CONFIG_PLAYER)) == 0)
            return goto_name(&st_rogue, &st_title, 0);
        else
    ...
    case TITLE_PACKAGES: return goto_state(&st_package);
...
static int title_gui(void)
{
    ...
    int sw = video.device_w;
    int sh = video.device_h;

    /*
     * Logo: load data/gui/title.png as the title image.
     * Dimensions are calculated to fill ~50% of screen width while
     * preserving the aspect ratio defined by LOGO_ASPECT_W:LOGO_ASPECT_H.
     * Change these two values to match your actual image dimensions.
     */
#define LOGO_ASPECT_W 4
#define LOGO_ASPECT_H 1

    int logo_w = sw / 2;
    int logo_h = logo_w * LOGO_ASPECT_H / LOGO_ASPECT_W;

    /* Never exceed 1/4 of screen height. */
    if (logo_h > sh / 4)
    {
        logo_h = sh / 4;
        logo_w = logo_h * LOGO_ASPECT_W / LOGO_ASPECT_H;
    }
    ...
    if ((root_id = gui_root()))
    {
        ...
        {
            /* Logo image — replaces the old text label. */
            if ((jd = gui_hstack(id)))
            {
                gui_filler(jd);
                gui_image(jd, "gui/title.png", logo_w, logo_h);
                gui_filler(jd);
            }
            ...
            if ((jd = gui_hstack(id)))
            {
                ...
                if ((kd = gui_varray(jd)))
                {
                    ...
                    gui_state(kd, gt_prefix("menu^More"),    GUI_MED, TITLE_HELP, 0);
...
#if ENABLE_VERSION
        if ((id = gui_label(root_id, "Ver 1.0", GUI_TNY, gui_wht2, gui_wht2)))
        {
            gui_clr_rect(id);
            gui_layout(id, -1, -1);
        }
