...
void progress_init(int m)
{
    ...
    if (mode == MODE_ROGUE && !rogue_get_balls_enabled())
        curr.balls = 0;    /* One life, no retry after a fall/time-out. */
    else
        curr.balls = 2;
...
int  progress_play(struct level *l)
{
    if (l)
    {
        ...
        /*
         * In roguelike mode treat every level as if its goal is unlocked
         * when balls are disabled; otherwise use normal challenge rules.
         */
        if (mode == MODE_ROGUE)
        {
            /* Open goal normally based on coins, never lock it behind
             * "completed" status since levels come from foreign sets. */
            goal_e = (goal == 0);
        }
        else
        {
...
void progress_stat(int s)
{
    ...
    case GAME_GOAL:
        ...
        /* Compute next level — not used in rogue mode (pool handles this). */

        if (mode == MODE_ROGUE)
        {
            /* next stays NULL; rogue pool manages sequencing. */
            next = NULL;
        }
    ...
    case GAME_TIME:
        ...
        /* In rogue mode with balls enabled, dying uses a ball. */
        /* The ball count was already decremented above. */

        break;
    }

    if (dirty && mode != MODE_STANDALONE && mode != MODE_ROGUE)
        set_store_hs();
...
int  progress_same_avail(void)
{
    ...
    {
    case GAME_NONE:
        return (mode != MODE_CHALLENGE && mode != MODE_ROGUE);

    default:
        if (mode == MODE_CHALLENGE || mode == MODE_ROGUE)
...
void progress_rename(int set_only)
{
    ...
    {
        /* Only rename level scores; there's no set-level high score. */
        level_rename_player(level, time_rank, goal_rank, coin_rank, player);
        demo_rename_player(USER_REPLAY_FILE, player);
        return;
    }
