...
static int audio_is_menu_track(const char *name)
{
    size_t len;
    ...
    return (len > 4 &&
            strncmp(name, "bgm/custom", 10) == 0 &&
            strcmp(name + len - 4, ".ogg") == 0);
}

void audio_music_fade_to_menu(float t)
{
    ...
    SDL_LockAudio();
    {
        if (audio_is_menu_track(music ? music->name : NULL) ||
            audio_is_menu_track(queue ? queue->name : NULL))
        {
            SDL_UnlockAudio();
            audio_music_fade_in(t);
            return;
    ...
    next = rand() % ARRAYSIZE(tracks);

    if (next == last)
        next = (next + 1) % ARRAYSIZE(tracks);

    last = next;

    audio_music_fade_to(t, tracks[next]);
}
